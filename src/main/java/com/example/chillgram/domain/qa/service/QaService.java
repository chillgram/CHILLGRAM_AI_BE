...
public class QaService {
    ...
    public Mono<Page<QaListResponse>> getQuestionList(int page, int size, String search, String status) {
        ...
        if (hasSearch && hasStatus) {
            ...
            listFlux = qaQuestionRepository.findByTitleContainingAndStatusOrBodyContainingAndStatusOrderByCreatedAtDesc(
        ...
        return qaQuestionRepository.findById(questionId)
                .flatMap(question -> {
                    ...
                    return Mono.zip(attachmentsMono, answersMono)
                            .map(tuple -> QaDetailResponse.from(question, tuple.getT1(), tuple.getT2()));
                })
                .switchIfEmpty(Mono.error(new IllegalArgumentException("Question not found id=" + questionId)));
    }
    ...
    public Mono<QaAnswerResponse> createAnswer(Long questionId, String body, Long companyId, Long answeredBy) {
        ...
        return qaQuestionRepository.findById(questionId)
                ...
                .doOnSuccess(
                        resp -> log.info("Answer created: questionId={}, answerId={}", questionId, resp.getAnswerId()))
