...
public class GcsFileStorage implements FileStorage {
    ...
    public Mono<byte[]> fetchBytes(String uri) {
        return Mono.fromCallable(() -> {
            ...
            if (blob == null) throw new IllegalStateException("gcs object not found: " + uri);
    ...
    public String toPublicUrl(String uri) {
        ...
        if (bucket != null && !bucket.isBlank() && !bkt.equals(bucket)) return u;
    ...
    private GcsLocation parseGcsLocation(String uri) {
        ...
        if (u.startsWith("gs://")) {
            String noScheme = u.substring("gs://".length());
            int slash = noScheme.indexOf('/');
            if (slash < 0) throw new IllegalArgumentException("invalid gs uri: " + u);
            String bkt = noScheme.substring(0, slash);
            String obj = noScheme.substring(slash + 1);
            if (obj.isBlank()) throw new IllegalArgumentException("invalid gs uri (empty object): " + u);
        ...
        if (u.startsWith("http://") || u.startsWith("https://")) {
            ...
            if (host == null) throw new IllegalArgumentException("invalid url: " + u);

            if ("storage.googleapis.com".equals(host)) {
                // /bucket/object
                if (path == null || path.length() < 2) throw new IllegalArgumentException("invalid gcs url: " + u);
                String p = path.substring(1);
                int slash = p.indexOf('/');
                if (slash < 0) throw new IllegalArgumentException("invalid gcs url: " + u);
                String bkt = p.substring(0, slash);
                String obj = p.substring(slash + 1);
                if (obj.isBlank()) throw new IllegalArgumentException("invalid gcs url (empty object): " + u);
                return new GcsLocation(bkt, obj);
            }
            ...
            if (host.endsWith(suffix)) {
                ...
                if (obj == null || obj.isBlank()) throw new IllegalArgumentException("invalid gcs url (empty object): " + u);
                return new GcsLocation(bkt, obj);
            }
